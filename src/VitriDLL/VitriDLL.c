...
LRESULT CALLBACK KbHookProc(int nCode, WPARAM wParam, LPARAM lParam) {
	...
	if (iTenths >= 0) {
		...
		if (iTenths > 0) {
			...
			SetWindowLong(hActiveWindow, GWL_EXSTYLE, GetWindowLong(hActiveWindow, GWL_EXSTYLE) | WS_EX_LAYERED);
			// Set the alpha value on it
			SetLayeredWindowAttributes(hActiveWindow, 0, (BYTE)(((float)iTenths/10)*255), LWA_ALPHA);
			/*
			...
			*/
			...
			SetWindowLong(hActiveWindow, GWL_EXSTYLE, GetWindowLong(hActiveWindow, GWL_EXSTYLE) & ~WS_EX_LAYERED);
			RedrawWindow(hActiveWindow, NULL, NULL, RDW_ERASE | RDW_INVALIDATE | RDW_FRAME | RDW_ALLCHILDREN);
		}
	}
